...
public class UserView extends VerticalLayout implements View{
	private static final long serialVersionUID = -9111877427053096441L;
...
//		addComponent(label);
		...
		Button newUser = new Button("Yeni Kullanıcı");
		...
		addComponent(newUser);
		 ...
		 MTable<User> userTable = 
				 ...
				 withProperties("id","adi","soyadi"
						 ,"kullaniciAdi","parola","magaza","role");
	...
	public void enter(ViewChangeEvent event) {
		...
		if(event.getParameters().length()>0){
			...
			if(user!=null){
				...
				Label label = new Label(user.getAdi() + user.getSoyadi());
				label.setStyleName(ValoTheme.LABEL_H1);
				Label label2 = new Label(user.getParola());
